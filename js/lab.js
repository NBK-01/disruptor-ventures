...
for (const [pos, contentItem] of contentItems.entries()) {
  ...
  contentItem.DOM.imgWrap.addEventListener("mouseleave", () => {
    ...
    gsap
      ...
      .addLabel("start", 0)
      .to(
        [contentItem.DOM.imgWrap, contentItem.DOM.img],
        {
          scale: 1,
        },
        "start"
...
backCtrl.addEventListener("click", () => {
  ...
  gsap
    ...
    .add(() => {
      const flipstate = Flip.getState(contentItem.DOM.imgWrap);
      contentItem.DOM.el.insertBefore(
        contentItem.DOM.imgWrap,
        contentItem.DOM.el.children[1]
      );
      Flip.from(flipstate, {
        duration: 0.8,
        ease: "power4.inOut",
...
// Preload images
preloadImages(".content__item-img").then(() =>
